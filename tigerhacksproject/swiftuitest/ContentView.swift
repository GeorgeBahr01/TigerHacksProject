...
struct ContentView: View {
    ...
    func getImportantHours(dayOfWeek:Int) -> [Int]
    {
        if(dayOfWeek == 0)
        {
            return data.sundayImportantHours
        }
        if(dayOfWeek == 1)
        {
            return data.mondayImportantHours
        }
        if(dayOfWeek == 2)
        {
            return data.tuesdayImportantHours
        }
        if(dayOfWeek == 3)
        {
            return data.wednesdayImportantHours
        }
        if(dayOfWeek == 4)
        {
            return data.thursdayImportantHours
        }
        if(dayOfWeek == 5)
        {
            return data.fridayImportantHours
        }
        else
        {
            return data.saturdayImportantHours
        }
    ...
    func buildNotification(importantHours: [Int], instant: Bool)
    {
        ...
        if defrost || precipitation || largeTempChange
        {
            ...
            if (instant)
            {
                dateComponents.second = (seconds + 2) % 60
                //overflow?
            }
            else
            {
    ...
    func getWeatherHours(importantHours: [Int]) -> [WeatherHours]
    {
        ...
        let url = URL(string: "https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/Columbia,MO/2022-11-4T00:00:00?key==temp,datetime,precipprob,preciptype,uvindex,cloudcover")!
    ...
    {
        let session = URLSession.shared
        let url = URL(string: "https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/Columbia,MO/2022-11-3T00:00:00?key=&elements=temp,datetime,precipprob,preciptype,uvindex,cloudcover")!
        var weatherHours: [WeatherHours] = []
        let group = DispatchGroup()
        group.enter()
        DispatchQueue.global().async {
